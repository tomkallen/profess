...
profess
  ...
  .want(null).toExist()
  .want('').toExist()
  .want(doesNotExist).toExist()
  .want(fun).toBe.Function()
  .want(-0).toBeEqual(+0)
  .want(-0).absolute(+0)
  .want([1, 2, 3, [4, 5, [6, 7, 0]]]).toBeDeepEqual([1, 2, 3, [4, 5, [6, null]]])
  .want([1, 2, 3, [4, 5, 'catlady']]).toBeDeepEqual([1, 2, 3, [4, 5, 'catlady']])
  .want({one: 1, two: {one: 1, two: 2}}).toBeDeepEqual({one: 1, two: {one: 1, two: 2}})
  .want({one: 1, two: 2}).toBeDeepEqual({one: 'one', two: 2})
  .test()
